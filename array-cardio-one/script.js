...
const oldestLived = inventors.sort((a, b) => {
  ...
  const secondPerson = b.passed - b.passed;
...
const de = links
              ...
              .filter((streetName) => {
                return streetName.indexOf('de') !== -1;
              });
console.log(de);
...
const alphabetical = people.sort((previousOne, nextOne) => {
  const [previousOneLast] = previousOne.split(', ');
  const [nextOneLast] = nextOne.split(', ');
  return previousOneLast > nextOneLast ? 1 : -1;
});
